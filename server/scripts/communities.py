...
class CreateUsers(BaseCommand):
    help = "Creates dummy posts"

    def handle(self, *args, **options) -> None:
        self.stdout.write(
            self.style.NOTICE("Creating dummy posts. This may take a while...")
        )
        with open(f"{JSON_DIR}/posts.json") as file:
            data = json.load(file)
        ...
        try:
            ...
            self.stderr.write(self.style.ERROR(f"Error creating dummy posts: {e}"))
        finally:
